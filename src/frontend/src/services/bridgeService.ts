...
function stringToChainType(chain: string): ChainType {
  switch (chain.toLowerCase()) {
    ...
    default: return { 'Ethereum': null }; // Default fallback
  }
}
...
function toCrossChainTransfer(bridgeRequest: BridgeRequest): CrossChainTransfer {
  return {
    ...
    completedAt: bridgeRequest.completed_at.length > 0 
...
export const bridgeService = {
   ...
   */
  async calculateBridgeFee(fromChain: string, amount: bigint): Promise<BridgeFeeEstimate> {
    try {
  ...
  validateTransfer(
    ...
    if (fromAddress && fromAddress.length < 10) errors.push('Invalid source address');
    if (toAddress && toAddress.length < 10) errors.push('Invalid destination address');
