...
const IdentityManager: React.FC = () => {
  ...
  if (isAuthenticated) {
    return (
      <div className="relative">
        ...
        {showDropdown && (
          <div className="ring-opacity-5 absolute right-0 mt-2 w-48 rounded-md bg-[#1b2127] shadow-lg ring-1 ring-black">
            <div className="py-1">
              <div className="border-b border-gray-600 px-4 py-2 text-xs text-gray-400">
                Connected with {walletType === "plug" ? "Plug" : "Oisy"} Wallet
              </div>
              <div className="px-4 py-2 font-mono text-xs text-gray-300">
                {principal?.toText()}
              </div>
              <button
                onClick={() => {
  ...
  return (
    <div className="relative">
      ...
      {showDropdown && (
        <div className="ring-opacity-5 absolute right-0 mt-2 w-56 rounded-md bg-[#1b2127] shadow-lg ring-1 ring-black">
          <div className="py-1">
            {error && (
              <div className="border-b border-gray-600 px-4 py-2 text-xs text-red-400">
                {error}
              </div>
            )}
