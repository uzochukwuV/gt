...
export function useOisyWallet() {
  ...
  const [icpMetadata, setIcpMetadata] = useState<any>();
  const [ckUsdcMetadata, setCkUsdcMetadata] = useState<any>();
  ...
  const transfer = async (ledger: IcrcLedgerCanister | null, metadata: any) => {
